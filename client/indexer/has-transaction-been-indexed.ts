...
const hasTxBeenIndexed = async (request: HasTxHashBeenIndexedRequest, client) => {
...
export const pollUntilIndexed = async (input: { txHash: string } | { txId: string }, client) => {
    while (true) {
        const response = await hasTxBeenIndexed(input, client)
